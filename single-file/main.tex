...
\begin{itemize}
	\item Zdefiniowanie regionu obejmującego każde normalne zachowanie jest trudne; granica między normą, a anomalią jest nieostra.
	\item Ewolucja normy: Wraz z czasem zmienia się percepcja, czym jest zachowanie normalne, co może dezaktualizować modele statyczne.
	\item Brak danych: W jednowymiarowych szeregach czasowych brak dobrze oznaczonych danych, co utrudnia ewaluację algorytmów i wymusza uczenie nienadzorowane.
\end{itemize}

\section{State Of The Art. - Metody}
Obecny stan wiedzy dzieli algorytmy na trzy główne grupy:
\begin{itemize}
	\item Tradycyjne metody statystyczne,
	\item Algorytmy uczenia maszynowego,
	\item Sieci neuronowe i głębokie uczenie.
\end{itemize}

\subsection{Metody statyczne i uczenie maszynowe}
\begin{itemize}
	\item \textbf{Metody statyczne:} opierają się na założeniu, że dane normalne występują w rejonach wysokiego prawdopodobieństwa modelu stochastycznego (system losowy w czasie), a anomalie niskiego.
	\item \textbf{Klasteryzacja i dystans (ML):} Algorytmy oparte na gęstości lub odległości. Zakładają, że normalne dane tworzą zwarte grupy, a anomalie występują w znacznej odległości od tych klastrów.
\end{itemize}

\subsection{Głębokie uczenie}
\begin{itemize}
	\item Sieci neuronowe najlepiej radzą sobie z wyciąganiem ukrytych, nieliniowych wzorców z sygnałów jednowymiarowych.
	\item Architektury rekonstrukcyjne: uczą się kompresowania i odtwarzania normalnych sygnałów; anomalią jest próbka, której nie można poprawnie zrekonstruować.
	\item Sieć uczy się kompresji i dekompresji na czystym sygnale.
	\item Usterką jest matematyczna różnica, tzw. błąd rekonstrukcji.
\end{itemize}

\section{Cel pracy}
\begin{itemize}
	\item Pomimo wielu rozwiązań nie istnieje uniwersalny algorytm do wszystkich typów danych.
	\item Celem jest zaimplementowanie i obiektywne porównanie skuteczności przedstawionych algorytmów różnych typów (modele statystyczne, ML, DL).
\end{itemize}\chapter{[Tytuł rozdziału]} 
